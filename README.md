...
<p align="center">
  <img src="frontend/images/icon2.png" alt="IsdaSure hero" width="920" style="max-width:100%; height:auto; object-fit:cover;" />
</p>

## IsdaSure — Community-First Micro-Insurance on Stellar (Soroban)

IsdaSure is a lightweight Soroban smart-contract dApp that lets coastal communities pool small, recurring contributions and receive immediate, transparent payouts when weather forces a no-fishing day. Built for real communities and constrained connectivity, IsdaSure prioritizes simplicity, resilience, and on-chain transparency.

---

## Quick Links

- Live demo: https://isda-sure.vercel.app
- Contract (testnet): CDNZVMTK3RNWWEQTG4JYC55O5P47YYTC2C2ACJVPI5MDJP63TH3KKKKS

---

## The Problem

Small-scale fisherfolk depend on daily catch income. When storms hit, earnings drop to zero and families rely on informal loans or delayed aid. Conventional insurance is expensive, slow, and inaccessible for micro-earnings. Communities need a fast, low-friction safety net that is trustable and locally governed.

## The IsdaSure Solution

IsdaSure enables community pooling and instant distribution using Soroban contracts:

- Participants contribute small daily amounts into a shared on-chain pool.
- A trusted local authority (e.g., barangay officer) triggers a `storm day` event.
- The Soroban contract verifies eligibility and distributes pooled funds equitably.
- All contributions and distributions are recorded on-chain for full auditability.

This approach replaces slow, centralized aid with immediate, transparent payouts owned by the community.

---

## Core Functions & Features

- On-chain group management: create/join groups, member lists, contribution rules.
- Contribution collection: validated daily contributions with configurable limits.
- Auto-storm payout: trigger distribution with deterministic payout logic.
- Local wallet integration: Freighter for signing and account interactions.
- Mock fallback: graceful UI/UX when RPC or contract is unavailable (mock confirmations).
- File-backed datastore: simple JSON storage with Vercel-friendly /tmp copy-on-start behavior.

---

## How It Works (overview)

1) Members create or join a group and set their identifiers and wallet addresses.
2) Each day members contribute a configured amount via the app (Freighter signs TX).
3) When a storm is declared, admin triggers `trigger_storm` which executes payout logic.
4) Contract computes and distributes payouts; frontend reflects on-chain state and history.

Simple flow diagram:

```
Member -> Contribute(amount) -> Contract: record contribution
Admin  -> TriggerStorm()    -> Contract: compute payouts -> distribute
```
...
```
IsdaSure/
├─ contract/           # Soroban (Rust) smart contract source and tests
├─ backend/            # Node/Express API, file-based datastore, RPC helpers
│  ├─ services/        # groupService, sorobanService, sorobanRpcService
│  └─ routes/          # api routes (contribute, groups, auth, triggerStorm)
├─ frontend/           # React + Vite app (UI, Freighter integration)
│  ├─ src/
│  └─ public/images    # icon2.png, logo, screenshots
└─ scripts/            # probes and diagnostics (check-contract.js)
```
...
- Browser (React + Vite): wallet integration and UI state mirroring the ledger.
- Backend (Node): lightweight API that prepares Soroban transactions, records activity, and provides a stable view for the UI.
- Soroban Smart Contracts: core pooling, contribution validation, and payout logic.
- Deployment: frontend + backend on Vercel, contracts on Stellar Testnet.
...
| Feature | Usage |
|---|---|
| Soroban Smart Contracts | Contract lifecycle: contributions, group state, payouts |
| XLM transfers / Fees | Low fee transactions, contract invocation metadata |
| Contract IDs & Explorer | Contract insights via Stellar Expert for tx links |
...
| Web App | Vercel (production alias) | https://isda-sure.vercel.app |
...
- Node.js v18+
- Rust toolchain for building Soroban contracts (stable + wasm target)
- `stellar-cli` for contract deployment (optional)
- Freighter browser extension for signing

---

## Smart Contract Setup & Testing (contract/)

1. Add WASM target and toolchain:
...
4. Deploy to testnet (example using stellar-cli):
...
2. Environment variables (create `.env` at project root or frontend env):
...
## Security, Error Handling & UX

- Contract-level guards validate callers and prevent double-spend or duplicate issuance.
- Backend provides graceful fallbacks when RPC or contract methods are unavailable:
  - `ALLOW_MOCK_ON_HOSTED` opt-in for hosted mock confirmations
  - frontend shows informative toasts when mock confirmations are used
- Freighter rejections and RPC account-not-found errors are surfaced cleanly to users.

---

## Test Snapshots & State Validation

The repository includes ledger snapshots used in automated contract tests to assert state transitions and ensure correctness across edge-cases (duplicate contributions, revocations, payout distribution).

---

## Contributing

Contributions welcome. Please open issues for feature requests and create pull requests against `main` for fixes. Follow the existing code style and run tests locally before submitting.
...
- Add a clickable table-of-contents at the top, or
- Commit this change locally for you to review (I will not push unless you ask), or
- Generate a prettier hero image and place it at `frontend/images/icon2.png`.
