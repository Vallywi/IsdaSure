<!--
IsdaSure README - detailed, Soroban-focused project documentation
Author: generated by assistant (edit as needed)
-->


<p align="center">
  <img src="frontend/images/logo.png" alt="IsdaSure" width="500" style="max-width:100%; height:auto; object-fit:cover; border-radius:8px;" />
</p>

[![Live Demo](https://img.shields.io/badge/Live_Demo-isda--sure-F59E0B?style=for-the-badge&logo=vercel&logoColor=white)](https://isda-sure.vercel.app/)
[![Stellar Testnet](https://img.shields.io/badge/Stellar-Testnet-7C3AED?style=for-the-badge&logo=stellar&logoColor=white)](https://stellar.expert/explorer/testnet/contract/CDNZVMTK3RNWWEQTG4JYC55O5P47YYTC2C2ACJVPI5MDJP63TH3KKKKS)
[![Soroban SDK](https://img.shields.io/badge/Soroban_SDK-22.0.0-3B82F6?style=for-the-badge)](https://docs.rs/soroban-sdk/22.0.0)
[![Vite](https://img.shields.io/badge/Vite-5.4.21-646CFF?style=for-the-badge&logo=vite&logoColor=white)](https://vitejs.dev/)
[![License: MIT](https://img.shields.io/badge/License-MIT-22C55E?style=for-the-badge)](LICENSE)


# IsdaSure

<p align="center"><strong>On-chain micro-insurance for coastal communities — Soroban smart contracts on Stellar Testnet.</strong></p>

---

## Overview 🧭

IsdaSure is a community-driven micro-insurance dApp that enables fisherfolk to pool small daily contributions into a shared on-chain fund. When a storm or extreme weather prevents fishing, the pooled funds are automatically distributed to contributors using a Soroban smart contract. The system prioritizes transparency, low friction, and accessibility for non-technical users.

Key goals:
- 💸 Low-cost, frequent micro-contributions
- 🔍 Transparent, auditable on-chain accounting
- 🤝 Community-controlled storm-trigger and fair payout distribution
- 🔐 Wallet-based UX via Freighter (signing) and lightweight backend for state aggregation

---

## Problem ❗

Small-scale fisherfolk depend on daily income from fishing. When storms occur they lose earnings and often resort to high-interest loans or delayed aid. Traditional insurance products are unaffordable or inaccessible. Communities need a simple, trustless system to pool tiny contributions and distribute funds quickly during no-fishing days.

---

## Solution 💡

IsdaSure uses Soroban smart contracts to store contributor state and execute payout logic when a community-authorized trigger occurs. The frontend integrates with Freighter to sign transactions; the backend provides a file-backed state and graceful fallbacks so the UX remains friendly even when RPC or network issues occur.

Benefits:
- Immediate, transparent distribution of pooled funds when a storm is triggered.
- Minimal friction: small daily contributions, Freighter signing, and straightforward join/contribute flows.
- No intermediaries — on-chain settlement and verifiable history.

---

## Core Functions & Features ✨

- 🔁 User flows: Register → Join/Create Group → Contribute → Receive payouts on Storm Day
- 🏷️ Group management: create groups, join requests, member approvals
- 📏 Contribution rules: configurable min/max, daily limits, and group-specific required daily amounts
- ⚡ Storm trigger: admin-authorized action that triggers on-chain payout calculation
- 🧩 Chain & mock modes: supports real RPC (when configured) and a mock fallback for unreliable RPCs

---

## How It Works (high-level) 🔁

1. Users register and join a named group.
2. Each day users make a small contribution (₱) via the app and sign with Freighter.
3. Contributions are recorded on-chain (or mocked when RPC unavailable) and stored in the app state.
4. When a storm day is declared by an authorized admin, the contract distributes funds to group members.

---

## How It Works — Detailed Flow 🔁

Below is a step-by-step ASCII flow that explains how contributions, prepare/submit, and storm-trigger distribution work. It includes the frontend, backend, and Soroban RPC interactions plus common failure/fallback paths so the flow remains understandable and actionable.

```
User (Freighter wallet)
  |
  |-- Register / Login ---------------------------+
  |  (frontend: Signup -> backend /users -> data/users.json)
  |
  |-- Create Group -------------------------------+   Admin / Group Owner
  |  (frontend POST /_/backend/api/groups/create) |   (optional approval flow)
  |                                                |
  |-- Join Group ---------------------------------+--> GroupService adds member (or creates join request)
  |  (frontend POST /_/backend/api/groups/join)   |    - `forceAddMember()` fallback allows wallet to contribute immediately
  |
  +-- Contribute Flow ----------------------------+
     1) Frontend: user clicks "Contribute" with `amount` and selected `groupId`.
     2) Frontend POST -> `/_/backend/api/contribute/prepare` with wallet address & amount.
        - Backend (`sorobanService.prepareTransactionWithFallback`) attempts to build Soroban `prepare` using `SOROBAN_RPC_URL`.
        - If RPC responds OK: returns unsigned prepared transaction (XDR) and `txNonce`.
        - If RPC errors (Account not found | method not found | DNS ENOTFOUND):
            - Backend returns a minimal mocked "prepared" payload and a flag `mode: 'mock'` OR a suggested `fallback: 'manageData'` flow.
     3) Frontend: signs the prepared tx using Freighter.
        - If prepared XDR available -> call Freighter signPreparedTransaction
        - If fallback recommended -> build a small `manageData` op and call Freighter.signTransaction
        - User may cancel at this stage -> frontend shows friendly cancellation toast and aborts.
     4) Frontend POST -> `/_/backend/api/contribute` with signed envelope/XDR and metadata.
        - Backend tries to `submitSignedSorobanTransaction` to Soroban RPC.
        - On SUCCESS:
            - Backend records confirmed tx in `chainHistory` and updates `data/pool.json` and `data/groups.json` contributions.
            - It sets `tx.mode = 'onchain'` and includes explorer URL if available.
        - On FAILURE (network / RPC submission error):
            - Backend creates a mocked confirmation entry (tx.mode = 'mock') with `note` explaining reason (e.g., "No Soroban RPC configured; using mocked confirmation") and still records contribution locally.
     5) Frontend displays toast: success (onchain) or info (mocked) including `backend.note` so user understands if action was simulated.

  +-- Daily State & Accounting --------------------+
     - Backend aggregates contributions per-group and per-user into `pool.json`.
     - Contribution history items reference `tx.id`, `tx.mode`, `amount`, and `confirmedAt` (mock or onchain).

Storm Trigger (Admin flow)
  |
  |-- Admin clicks "Trigger Storm" for a Group
  |-- Frontend -> `/_/backend/api/triggerStorm/prepare`
  |     - Backend prepares a Soroban `trigger_storm` call (or returns mock prepared payload when RPC unavailable)
  |-- Admin signs prepared tx with Freighter
  |-- Frontend -> `/_/backend/api/triggerStorm/submit`
  |     - Backend attempts RPC submission and waits for confirmation
  |     - On success -> contract executes distribution: calculates per-member payout and transfers via contract.
  |     - Backend records distribution in `chainHistory` and updates member balances in `data/groups.json`.
  |     - On failure -> backend records a mocked distribution entry with explanatory `note` and still distributes locally according to fallback rules.

Edge Cases & Failure Paths
  - Freighter not installed / user cancels signing -> frontend aborts and shows clear action guidance.
  - RPC returns `Account not found` when preparing: backend suggests `manageData` fallback so the wallet can still sign a small proof-of-possession and contribute.
  - RPC `method not found` (contract not deployed or RPC mismatch): backend returns a helpful message and a `mock` prepared tx so UX isn't blocked.
  - DNS / ENOTFOUND: treat as transient network failure -> return mock prepare with `note` and record mock submission if needed.
  - Hosted runtime (Vercel): file system is ephemeral -> packaged `backend/data/*.json` copied to `/tmp/isdasure-data` on first run so admin UI sees seeded groups/users.

Observability & UX
  - Each contribution and trigger is recorded in `chainHistory` with `mode: 'onchain'|'mock'`, `note`, and optional `explorerUrl`.
  - Frontend surfaces `mode` and `note` in toasts and in `ContributionHistory` list so users can tell if an action was mocked.
  - Admin pages show when the app is running in mock mode (based on environment vars and RPC availability) so maintainers can decide to set `SOROBAN_RPC_URL` and `SOROBAN_CONTRACT_ID`.

``` 


## Project Structure 📁

```
IsdaSure/                       📦 Project root
├─ backend/                     🖥️  Express API, services, file-backed data
│  ├─ app.js                    🚀  Express app entry
│  ├─ server.js                 ▶️  HTTP server / startup
│  ├─ routes/                   🔁  API route handlers (auth, contribute, groups)
│  ├─ services/                 ⚙️  Business logic (sorobanService, groupService, rpc)
│  └─ data/                     📄  Seeded JSON (groups.json, pool.json, users.json)
├─ contract/                    🦀  Soroban Rust smart contract source & tests
│  ├─ src/                      🧩  Rust sources (lib.rs, test.rs)
│  └─ test/                     🧪  Contract tests & snapshots
├─ frontend/                    🎛️  React + Vite frontend (Freighter integration)
│  ├─ src/                      🧭  Components, pages, context, hooks, services
│  ├─ public/                   🖼️  Static assets & favicon (images/icon2.png)
│  └─ package.json              📦  Frontend deps & scripts
├─ scripts/                     🔧  Utilities (contract checks, diagnostics)
├─ test_snapshots/              📸  Contract test snapshots used for regression tests
└─ README.md                    📘  This file
```

---

## Architecture 🏗️

- Frontend: React + Vite, integrates with Freighter for signing, and calls backend APIs under `/_/backend/api` when hosted.
- Backend: Node.js + Express — manages groups, contributions, and prepares/submits Soroban transactions via the `SOROBAN_RPC_URL` (if configured). Uses packaged JSON files as seeded data and copies to a writable `/tmp` folder on serverless hosts.
- Contract: Soroban Rust contract manages contribution accounting and storm-trigger payouts.

---

## Deployment & Contract Addresses 🚢

| Layer | Environment | Value |
|---|---:|---|
| Contract | Stellar Testnet | `CDNZVMTK3RNWWEQTG4JYC55O5P47YYTC2C2ACJVPI5MDJP63TH3KKKKS` |
| Network Passphrase | Testnet | `Test SDF Network ; September 2015` |
| Live Web App | Vercel | https://isda-sure.vercel.app |

---

## Prerequisites 🛠️

- Node.js v18+
- Rust toolchain (for building Soroban contract) and `wasm32-unknown-unknown` target
- Soroban CLI (recommended for contract deploys)
- Freighter browser wallet configured for Stellar Testnet

---

## Smart Contract: Build / Test / Deploy (contract/) 🦀

1. Install the wasm target and build toolchain

```bash
rustup target add wasm32-unknown-unknown
```

2. Build the contract

```bash
cd contract
stellar contract build
```

3. Run tests

```bash
cargo test
```

4. (Optional) Deploy to testnet with Soroban / Stellar CLI

```bash
stellar keys generate --global my-admin-key --network testnet
stellar keys fund my-admin-key --network testnet
cd contract
stellar contract deploy --wasm target/wasm32-unknown-unknown/release/isdasure.wasm \
  --source my-admin-key --network testnet
```

---

## Frontend: Local Setup (frontend/) 🎛️

1. Install dependencies

```bash
cd frontend
npm install
```

2. Configure `.env` (local development)

```
VITE_SOROBAN_CONTRACT_ID=CDNZVMTK3RNWWEQTG4JYC55O5P47YYTC2C2ACJVPI5MDJP63TH3KKKKS
VITE_RPC_URL=https://soroban-testnet.stellar.org
VITE_NETWORK_PASSPHRASE="Test SDF Network ; September 2015"
```

3. Run locally

```bash
npm run dev
```

Notes: The UI integrates with Freighter for signing. If `SOROBAN_RPC_URL` is missing on hosted environments, the backend falls back to a mock confirmation mode when `ALLOW_MOCK_ON_HOSTED=true`.

---

## API Endpoints (backend) 🔌

- `POST /_/backend/api/status` — get current pool, groups, chainHistory
- `POST /_/backend/api/contribute/prepare` — prepare unsigned Soroban tx for contribution
- `POST /_/backend/api/contribute` — submit signed tx / record contribution
- `POST /_/backend/api/groups` — list groups
- `POST /_/backend/api/groups/create` — create group

See `backend/routes` for full details.

---

## Testing & Snapshots 🧪

Contract tests use JSON snapshots under `test_snapshots/` to verify state transitions. Run `cargo test` to execute.

---

## Notes on Hosted/RPC behavior ☁️

- Vercel has an ephemeral filesystem. Packaged `backend/data/*.json` files are copied into `/tmp` on first run so admin UI can display seeded groups/users.
- Soroban RPC nodes may return `Account not found` for wallets that have not been funded on testnet; the app provides a friendly fallback that allows wallets to sign a lightweight manageData tx so contributions can still be associated with a wallet.

---

## Contributing 🤝

Contributions are welcome — open an issue or a pull request. Please follow the repository's coding style and add tests for contract logic when changing `contract/src`.

---

## License 📜

This project is licensed under the MIT License — see the `LICENSE` file for details.
